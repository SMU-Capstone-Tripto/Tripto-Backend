...
async def agent_chat(
    ...
    **이벤트 종류:**
    ...
    - `{"type": "result", "content": "...", "step": "optimized", ...}` — 최종 결과
    - `{"type": "error", "message": "..."}` — 오류
    - `[DONE]` — 스트림 종료 신호
    """
    return StreamingResponse(
        agent_service.chat_stream(current_user.user_id, body.message),
...
async def reset_session(current_user: User = Depends(get_current_user)):
    ...
    agent_service.reset_session(current_user.user_id)
...
async def get_session_state(current_user: User = Depends(get_current_user)):
    ...
    session = agent_service.get_session(current_user.user_id)
